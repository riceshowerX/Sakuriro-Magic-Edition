...
if( !empty($_POST['register_reg']) ) {
  ...
  if($error == '') {
    ...
    if ( !$user_id ) {
      ...
      do_action('wp_login', $sanitized_user_login);
      // 跳转到成功页面
      $redirect_url = get_permalink( get_page_by_path( 'success' ) ); 
  ...
  <?php if(!is_user_logged_in()){ ?>
    <div class="ex-register">
      ...
      <form action="<?php echo $_SERVER["REQUEST_URI"]; ?>" method="post">  
        ...
        <p><input type="email" name="user_email" tabindex="2" id="user_email" class="input" value="<?php if(!empty($user_email)) echo $user_email; ?>" size="25" placeholder="<?php _e("Email Address","sakurairo");?>" required /></p>
